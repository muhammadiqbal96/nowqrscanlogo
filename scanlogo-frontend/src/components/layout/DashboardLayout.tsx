...
import {
    LayoutDashboard, Megaphone, QrCode, BarChart3, Settings, CreditCard, Sticker, Package,
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="p-4 border-t border-border">
                <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-sm overflow-hidden relative">
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <button className="flex items-center gap-2 p-2 rounded-lg hover:bg-muted transition-colors">
                                    <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-xs overflow-hidden relative">
                                        <span className="select-none">{user?.first_name?.[0]}{user?.last_name?.[0]}</span>
                                        {user?.avatar && (
                                            <img src={user.avatar} alt="" className="absolute inset-0 w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none' }} />
                                        )}
