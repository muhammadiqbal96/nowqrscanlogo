...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [token, setToken] = useState<string | null>(
    () => localStorage.getItem('scanlogo_token')
  );
  ...
  const saveAuth = (userData: User, authToken: string) => {
    setUser(userData);
    setToken(authToken);
    localStorage.setItem('scanlogo_token', authToken);
    localStorage.setItem('scanlogo_user', JSON.stringify(userData));
  };

  const clearAuth = () => {
    setUser(null);
    setToken(null);
    localStorage.removeItem('scanlogo_token');
    localStorage.removeItem('scanlogo_user');
  };
  ...
  const refreshUser = useCallback(async () => {
    try {
      ...
      localStorage.setItem('scanlogo_user', JSON.stringify(response.data.user));
    } catch {
      clearAuth();
    }
  }, []);
  ...
  const setAuthFromToken = async (newToken: string) => {
    localStorage.setItem('scanlogo_token', newToken);
    setToken(newToken);
    try {
      const response = await authApi.me();
