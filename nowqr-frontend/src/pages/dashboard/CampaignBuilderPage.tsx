...
export default function CampaignBuilderPage() {
  ...
  const handleSaveCampaign = async () => {
    ...
    try {
      ...
      toast.success('Campaign created! Now let\'s build your ScanLogo.')
      // Flow Step 5: Automatically go to ScanLogo builder
      navigate(`/dashboard/scanlogos/new?campaign_id=${campaignId}`)
    } catch (err: any) {
            ...
            {CTA_OPTIONS.map((option) => (
              <button
                ...
                className={`p-5 rounded-2xl border-2 text-left transition-all hover:shadow-lg group ${
                  ctaType === option.value ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50'
                }`}
              >
          ...
          <div className="grid lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-2 space-y-5">
                  ...
                  {FONT_OPTIONS.map((font) => (
                    <button
                      ...
                      className={`px-3 py-2 text-xs rounded-lg border transition-all ${
                        fontFamily === font ? 'border-primary bg-primary/5 text-primary' : 'border-border hover:border-primary/50'
                      }`}
              ...
              <div className="flex gap-3 pt-4">
                ...
                >
                  {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <>Save & Build ScanLogo <ArrowRight className="w-4 h-4" /></>}
                </button>
              </div>
            </div>
