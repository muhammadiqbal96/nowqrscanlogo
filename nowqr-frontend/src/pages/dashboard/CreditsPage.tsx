        ...
        <div className="max-w-5xl mx-auto space-y-8">
            <div>
                ...
                <p className="text-muted-foreground text-sm mt-1">
                    {isAdmin
                        ? 'Admin accounts have unlimited credit usage across ScanLogos and canvas flows.'
                        : 'Buy credits to create campaigns and ScanLogos. No subscriptions.'}
                </p>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-6 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-3xl font-bold">{isAdmin ? 'Unlimited' : (user?.credits ?? 0)}</p>
                        <p className="text-sm text-muted-foreground">{isAdmin ? 'Admin credit access' : 'Available credits'}</p>
                    </div>
                </div>
                {isAdmin ? (
                    <div className="px-4 py-2 text-xs font-semibold rounded-xl bg-primary/10 text-primary">
                        Purchases disabled for admin
                    </div>
                ) : (
                    <button
                        onClick={handleTopUp}
                    ...
                    {CREDIT_COSTS.map((item) => (
                        <div key={item.action} className="flex items-center gap-3 p-3 bg-muted/50 rounded-xl">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-primary font-bold">{isAdmin ? 'No charge (admin)' : `${item.cost} credits`}</p>
                            </div>
                        </div>
                    ))}
                </div>
